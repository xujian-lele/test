 ...
 * 
 * @author xujian
 *	将中缀表达式转换成前缀表达式 :( a + b ) * (c + d)  转换成 * + a b + c d
 */
public class Nifix_2_Prefix implements ExpressionProcessor{
			...
			if(Operators.INSTANCE.isOperator(word)){
				if(word.equalsIgnoreCase("(")){
					...
					if(wordPriority >= optPriority){
						...
						//输入符优先级别低
						while(!optStack.isEmpty() && wordPriority < getPriority(optStack.peek().toString())){
	 ...
	 * @param numStack	数据栈
	 * @param optStack	操符栈
	 * function ： 从数据栈中弹出最顶端的两个元素b，a；从操作符栈中淡出最顶端的那个元素o；将oab 加入到数据栈中。
	 */
	private String popStack(Stack<String> numStack, Stack<String> optStack) {
		if(numStack.isEmpty() || numStack.size()==1) return "";
		if(optStack.isEmpty()){
